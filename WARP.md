...
- **Install/run on device or emulator**
  ...
  - After install, launch the "MonoMusic" app from the launcher.
...
- **`MonoMusic` `Application` class** (`app/src/main/java/com/calmapps/calmmusic/MonoMusicApplication.kt`)
  ...
  - Creates and exposes (via `lateinit` properties) the core Apple Music integration objects:
    ...
    - `MonoMusicSettingsManager` – manages user settings (local music on/off, folders) via SharedPreferences + `StateFlow`s.
  - The `MainActivity` accesses these singletons by casting `application` to `MonoMusic`.
...
- **Settings** (`MonoMusicSettingsManager.kt`)
...
- **Local playback service** (`PlaybackService.kt`)
  ...
  - Exposes the session to clients; `MainActivity`/`MonoMusic` obtain a `MediaController` via `SessionToken` and use it as `localMediaController`.

- **Apple Music playback** (`AppleMusicCore.kt`, `MonoMusicApplication.kt`, `MainActivity.kt`)
...
- **Shared playback queue and state** (`MainActivity.kt` – `MonoMusic` composable)
...
- **Entry point** (`MainActivity.kt`)
  - `MainActivity` hosts a single `MonoMusic` composable in `setContent`, wrapped with the Mudita MMD `ThemeMMD`.
...
- **Screen composables** (`app/src/main/java/com/calmapps/calmmusic/ui/`)
  ...
  - They receive data as UI models and callbacks for user actions; most side effects and data persistence are handled in `MonoMusic`.
...
- **Adjusting Apple Music vs local library sync**
  - Apple Music library sync logic and error handling live in `MonoMusic` (within `MainActivity.kt`), using `AppleMusicApiClient` and the DAOs.
  - Local library sync is triggered from `MonoMusic` based on `MonoMusicSettingsManager` flows and implemented via `LocalMusicScanner` and Room.
